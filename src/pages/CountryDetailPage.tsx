...
export function CountryDetailPage() {
  ...
  const { data: country, isLoading, isError, error, refetch } = useCountryDetails(code);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {isLoading ? (
        ...
        <ErrorState
          message={error instanceof Error ? error.message : "Country not found."}
          onRetry={() => refetch()}
        />
      ) : (
        <article className="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-700">
          <div className="flex max-h-56 items-center justify-center overflow-hidden bg-slate-100 dark:bg-slate-700">
            <img src={country.flagPngUrl} alt="" className="w-full object-cover" />
          </div>
