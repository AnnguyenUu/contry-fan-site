...
- **Skeleton placeholders, not a spinner, for loading states.** `CountryGridSkeleton`/`CountryCardSkeleton`/`CountryDetailSkeleton` (`src/modules/countries/presentation/`) reserve the same box dimensions as the real grid/card/article they stand in for (same aspect ratios, same image height, same text-line heights), built on a shared `Skeleton` primitive (`src/shared/presentation/Skeleton.tsx`). A centered spinner sizes nothing like the content that replaces it, so it was actively hurting Cumulative Layout Shift; matching placeholder boxes mean the layout is already correct before data arrives, and only pixel content repaints in place. Screen readers get the same information via `<output aria-label="…">` wrapping the (otherwise `aria-hidden`) skeleton, instead of visible loading text.
- **A class-based `ErrorBoundary`** (`src/shared/presentation/ErrorBoundary.tsx`), reusing `ErrorState` for its fallback, at two levels: around the routed page content in `AppLayout` (keyed by pathname, so navigating away from a page that crashed remounts and recovers it automatically) and once more around the whole app in `App.tsx` as a last-resort net for errors thrown outside the router's tree (e.g. in a context provider). React Query already handles *expected* failures (network errors, 4xx/5xx) via `isError`; the boundary exists for genuinely unexpected render-time bugs, so one broken page shows a recoverable error instead of a blank white screen.
